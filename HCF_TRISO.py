...
# After add the cladding model of MCNP about the HCF
# then we should to make more fidelity model of the HCF,
# we need to put the TRISO with hexagonal lattice in it. 
# So, here is the modification: add the TRISO hexagonal 
# lattice number : 998 in the HCF region.
...
def Main(plate_cnt, 
    ...
    with open("surface.txt", "a") as f_surface, open("cell.txt", "a") as f_cell:
        ...
        for i in range(plate_cnt):
            ...
            ###############################################################
            ''' inner HCF model '''
            # the HCF has been filled by TRISO particles which dispersion in graphite matrix. 
            string = "%d 0 %d %d %d %d %d -%d & \n" % (cell_cnt, 
            ...
            string = string + "     :(-%d %d -%d):(-%d %d -%d): & \n      (-%d %d -%d):(-%d %d -%d) u=1 fill=%d imp:n=1\n" % (
...
if __name__ == '__main__':
    # the TRISO particle universe
    TRISO_UNIVERSE = 998
    section_number = 100
    surface_cnt, cell_cnt, original_surface_cnt = 1, 1, 1
    surface_cnt, cell_cnt, original_surface_cnt = Main(section_number, 
                                                       1.05*math.sqrt(2), 1.05, 
                                                       0,   80,  
                                                       0.78, 0.27, 
                                                       0.7, 0.35, 
                                                       surface_cnt, cell_cnt, original_surface_cnt, 
