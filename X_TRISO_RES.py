...
''' Add the TRISO model and the BP model of MCNP '''
...
def Main(plate_cnt, 
    ...
    if material_type == 1:  # HCF
        universe = 1
        particle_universe = 998    # TRISO universe
    elif material_type == 2: # posion rod
        universe = 4
        particle_universe = 996    # BP universe
    ...
    with open("surface.txt", "a") as f_surface, open("cell.txt", "a") as f_cell:
        ...
        for i in range(plate_cnt):
            ...
            #                       CELL CARD                             #
            ###############################################################
            # judge the type is there any filled particles.
            if material_type == 1 or material_type == 2:
                ...
                string = string + "     :(-%d %d -%d):(-%d %d -%d): & \n      (-%d %d -%d):(-%d %d -%d) u=%d fill=%d imp:n=1\n" % (
                          ...
                          universe,
                          particle_universe)
            # the graphite rod, which not contains the particles.
            else:
            ...
            string = string + "%d 2 -1.94 -99002 99005 -99007 99004 -99006 99003 & \n     %d -%d #%d #%d u=%d imp:n=1\n" % (cell_cnt, 
...
if __name__ == '__main__':
    ...
    for i in range(5):
        surface_cnt, cell_cnt, original_surface_cnt, record_original_surface_cnt = Main(plate_cnt, 
                                                           1.2001*math.sqrt(2), 1.2001, 
                                                           0.05+i*d_height,   0.05+(i+1)*d_height,
                                                           0.835, 0.365, 
                                                           0.755, 0.445, 
                                                           surface_cnt, cell_cnt, original_surface_cnt,
                                                           HCF)
