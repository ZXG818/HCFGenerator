...
def Main(plate_cnt, R, r, low, high, diag_radius, axis_raidus, clad_diag_radius, clad_axis_radius, surface_cnt, cell_cnt, original_surface_cnt):
    with open("surface.txt", "a") as f_surface, open("cell.txt", "a") as f_cell:
        ...
        for i in range(plate_cnt):
            ...
            surface_cnt += 1                                                                      
            f_surface.write("%d P %f %f %f %f\n" % (surface_cnt, A4, B4, 0.0, D4))                # original_surface_cnt + 4
            surface_cnt += 1                                                                      
            f_surface.write("%d PZ %f\n" % (surface_cnt, low + i*d_h))                            # original_surface_cnt + 5
            surface_cnt += 1                                                                      
            f_surface.write("%d PZ %f\n" %(surface_cnt, low + (i+1)*d_h))                         # original_surface_cnt + 6
            surface_cnt += 1

            # large cylinder which is INSIDE.
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, X1, Y1, diag_radius))             # original_surface_cnt + 7
            surface_cnt += 1
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, X2, Y2, diag_radius))             # original_surface_cnt + 8
            surface_cnt += 1
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, X3, Y3, diag_radius))             # original_surface_cnt + 9
            surface_cnt += 1
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, X4, Y4, diag_radius))             # original_surface_cnt + 10
            surface_cnt += 1

            # litte cylinder which is INSIDE.
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, x1, y1, axis_raidus))             # original_surface_cnt + 11
            surface_cnt += 1
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, x2, y2, axis_raidus))             # original_surface_cnt + 12
            surface_cnt += 1
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, x3, y3, axis_raidus))             # original_surface_cnt + 13
            surface_cnt += 1
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, x4, y4, axis_raidus))             # original_surface_cnt + 14
            surface_cnt += 1
            
            # large cylinder which is OUTSIDE.
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, X1, Y1, clad_diag_radius))        # original_surface_cnt + 15
            surface_cnt += 1
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, X2, Y2, clad_diag_radius))        # original_surface_cnt + 16
            surface_cnt += 1
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, X3, Y3, clad_diag_radius))        # original_surface_cnt + 17
            surface_cnt += 1
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, X4, Y4, clad_diag_radius))        # original_surface_cnt + 18
            surface_cnt += 1
            
            # little cylinder which is OUTSIDE.
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, x1, y1, clad_axis_radius))        # original_surface_cnt + 19
            surface_cnt += 1
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, x2, y2, clad_axis_radius))        # original_surface_cnt + 20
            surface_cnt += 1
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, x3, y3, clad_axis_radius))        # original_surface_cnt + 21
            surface_cnt += 1
            f_surface.write("%d C/Z %f %f %f\n" % (surface_cnt, x4, y4, clad_axis_radius))        # original_surface_cnt + 22
            surface_cnt += 1
...
if __name__ == '__main__':
    ...
    surface_cnt, cell_cnt, original_surface_cnt = Main(5, 1.05*math.sqrt(2), 1.05, 0,   80,  0.78, 0.27, 0.7, 0.35, surface_cnt, cell_cnt, original_surface_cnt)
    surface_cnt, cell_cnt, original_surface_cnt = Main(5, 1.05*math.sqrt(2), 1.05, 80,  160, 0.78, 0.27, 0.7, 0.35, surface_cnt, cell_cnt, original_surface_cnt)
    surface_cnt, cell_cnt, original_surface_cnt = Main(5, 1.05*math.sqrt(2), 1.05, 160, 240, 0.78, 0.27, 0.7, 0.35, surface_cnt, cell_cnt, original_surface_cnt)
    surface_cnt, cell_cnt, original_surface_cnt = Main(5, 1.05*math.sqrt(2), 1.05, 240, 320, 0.78, 0.27, 0.7, 0.35, surface_cnt, cell_cnt, original_surface_cnt)
    surface_cnt, cell_cnt, original_surface_cnt = Main(5, 1.05*math.sqrt(2), 1.05, 320, 400, 0.78, 0.27, 0.7, 0.35, surface_cnt, cell_cnt, original_surface_cnt)
    print("Done the work...\n");
