...
# main function
def Main(plate_cnt, R, r, low, high, diag_radius, axis_raidus, surface_cnt, cell_cnt, original_surface_cnt):
    with open("surface.txt", "a") as f_surface, open("cell.txt", "a") as f_cell:
        ...
        for i in range(plate_cnt):
            ...
            ###############################################################
            '''
            string = "%d %d -9.8 %d %d %d %d %d -%d & \n" % (cell_cnt, cell_cnt,
                      PlatePositive(x1, y1, A1, 1.0, -D1)*(original_surface_cnt+0),
                      PlatePositive(x2, y2, A2, 1.0, -D2)*(original_surface_cnt+1),
                      PlatePositive(x3, y3, A3, 1.0, -D3)*(original_surface_cnt+2),
                      PlatePositive(x4, y4, A4, 1.0, -D4)*(original_surface_cnt+3),
                      original_surface_cnt+4,
                      original_surface_cnt+5)'''
            string = "%d 1 -2.6593 %d %d %d %d %d -%d & \n" % (cell_cnt, 
            ...
            cell_cnt += 1
            string = string + "%d 2 -1.94 -%d %d -%d #%d u=1 imp:n=1\n" % (cell_cnt, 
                                                                     9000,
                                                                     original_surface_cnt+4, original_surface_cnt+5, 
                                                                     cell_cnt-1)
            
            cell_cnt += 1
            original_surface_cnt += 14
            f_cell.write(string)
    return surface_cnt, cell_cnt, original_surface_cnt


if __name__ == '__main__':
    # Main(36, math.sqrt(2), 1, 400, 0.6, 0.4)  # test the program
    surface_cnt, cell_cnt, original_surface_cnt = 1, 1, 1
    surface_cnt, cell_cnt, original_surface_cnt = Main(8, 1.05*math.sqrt(2), 1.05, 0,   80,  0.7, 0.35, surface_cnt, cell_cnt, original_surface_cnt)
    surface_cnt, cell_cnt, original_surface_cnt = Main(8, 1.05*math.sqrt(2), 1.05, 80,  160, 0.7, 0.35, surface_cnt, cell_cnt, original_surface_cnt)
    surface_cnt, cell_cnt, original_surface_cnt = Main(8, 1.05*math.sqrt(2), 1.05, 160, 240, 0.7, 0.35, surface_cnt, cell_cnt, original_surface_cnt)
    surface_cnt, cell_cnt, original_surface_cnt = Main(8, 1.05*math.sqrt(2), 1.05, 240, 320, 0.7, 0.35, surface_cnt, cell_cnt, original_surface_cnt)
    surface_cnt, cell_cnt, original_surface_cnt = Main(8, 1.05*math.sqrt(2), 1.05, 320, 400, 0.7, 0.35, surface_cnt, cell_cnt, original_surface_cnt)
    #MaterialCard("material.txt");
    print("Done the work...\n");